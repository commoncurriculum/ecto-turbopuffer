...
Branches inherit the source namespace's [`read_only`](/docs/metadata#change-metadata)
setting when created. The setting can be changed independently on the branch
afterward. Other per-namespace configuration like [pinning](/docs/pinning) is
not inherited; the branch starts unpinned.
