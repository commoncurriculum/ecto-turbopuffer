...
A namespace that would exceed a single index can be [sharded](/docs/sharding):
its documents are partitioned across multiple internal indexes that share the
same WAL, with writes and queries fanned out and merged transparently.
