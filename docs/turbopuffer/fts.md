...
The `pre_tokenized_array` tokenizer does not support [highlighting](#highlighting).
...
turbopuffer currently supports language-aware stemming for the following
languages:
...
Stopword removal is supported for all except `arabic`, `greek`, `romanian`,
`tamil`, and `turkish`.
...
Highlighting is in beta, and is free during the beta period.

Highlighting splits an attribute's text into fragments — words, sentences,
paragraphs, or the whole value — and returns the fragments most relevant to the
query. Use it to show users *why* a result matched, or to hand an agent the relevant
fragments of a large document instead of the whole thing.

Add a `Highlight` expression to
[`compute_attributes`](/docs/query#param-compute_attributes) on any query. The
highlighted `string` or `[]string` attribute must have full-text search
enabled:

```jsonc
{
  "rank_by": ["text", "BM25", "write-ahead log turbopuffer"],
  "compute_attributes": {
    "matching_fragments": ["Highlight", "text"]
  },
  "limit": 10
}
```

The `matching_fragments` key is user-defined (here, `matching_fragments`) and
will be used as the field name in each returned document. The fragments for each
returned document are scored and ranked using either the query's `rank_by` expression
(default) or an expression defined by `rank_fragment_by`, and the top `fragment_limit`
fragments for each document are returned in the query response. Setting
[`include_offsets`](/docs/query#param-compute_attributes) adds each fragment's
position and the positions of the matching text within that fragment as half-open
`[start, end)` ranges.

When choosing `include_offsets`, match your language's string representation:
`utf-8` (the default) for Rust and Go, `utf-16` for JavaScript and Java,
`codepoints` for Python.

For agents searching large documents, highlighting replaces retrieving whole
documents with retrieving the fragments that matter. Paragraph fragments give
the agent enough context to reason with, and `fragment_range` lets it fetch
surrounding text or cite its source later:

```jsonc
{
  "rank_by": ["content", "BM25", "termination for convenience"],
  "compute_attributes": {
    "relevant_fragments": ["Highlight", "content", {
      "fragment_by": "paragraph",
      "fragment_limit": 10
    }]
  },
  "limit": 5
...
To get match positions without splitting the text into fragments, set
`fragment_by` to `none`: the whole attribute becomes one fragment and
`match_ranges` lists every match in it.

See the [query reference](/docs/query#param-compute_attributes) for all options
and defaults.
