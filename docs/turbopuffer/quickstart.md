...
2. Create an [API key](https://turbopuffer.com/dashboard) from the Dashboard.
   ...
   your preferred region if needed. Not every model runs in every region, so
   if you change either, check the [model list](/docs/embedding#models).
3. The snippets use [native embeddings](/docs/embedding), so turbopuffer
   embeds your text on writes and queries and you don't integrate an embedding
   provider at all. To call a provider from your own code instead, pick one
   from the embedding provider
   dropdown in the code sample below.
...
```
```go
// $ go get github.com/turbopuffer/turbopuffer-go/v2 github.com/cohere-ai/cohere-go/v2
package main
...
```
```go
// $ go get github.com/turbopuffer/turbopuffer-go/v2 google.golang.org/genai
package main
...
```
```go
// $ go get github.com/turbopuffer/turbopuffer-go/v2 github.com/openai/openai-go
package main
...
import (
	...
	"github.com/turbopuffer/turbopuffer-go/v2"
	tpufoption "github.com/turbopuffer/turbopuffer-go/v2/option"
)
...
[Upsert](/docs/write) documents with typed attributes and
[full-text search](/docs/fts) on `text` and `category` (with
[regex](/docs/query#param-Regex) on `text`). Setting `embed` on `text` turns it
into a vector on every write, so no vectors go over the wire.
    ...
    new NamespaceWriteParams
    {
        ...
        Schema = new Dictionary<string, AttributeSchemaConfig>
        {
            ["text"] = new AttributeSchemaConfig
            {
                ...
                FullTextSearch = true,
                Regex = true,
                Embed = new AttributeEmbedConfig { Model = "nvidia/nemotron-3-embed-8b", Dims = 1024 },
            },
...
ns.write(
  upsert_rows: [
    { id: 1, category: ["mammal"], public: true,
      text: "walrus narwhal" },
    { id: 2, category: ["fish"], public: false,
      text: "pufferfish clownfish swordfish" },
  ],
  distance_metric: "cosine_distance",
  schema: {
...
Find documents by [vector similarity](/docs/vector) with filters, where
`Embed` embeds the query text with the model configured on `text`. Or search
...
curl $TPUF_URL/v2/namespaces/$TPUF_NAMESPACE/query \
  ...
  -H 'Content-Type: application/json' \
  -d '{
   "rank_by": ["text", "ANN", ["Embed", "arctic sea mammal"]],
   "limit": 10,
...
```go
	...
	res, err := ns.Query(ctx, turbopuffer.NamespaceQueryParams{
		RankBy:  turbopuffer.NewRankByAnnExpr("text", turbopuffer.NewExprEmbed("arctic sea mammal")),
...
```java
    ...
    var queryResult = ns.query(
      NamespaceQueryParams.builder()
        .rankBy(RankBy.ann("text", Expr.embed("arctic sea mammal")))
        .limit(10)
...
ns.write(
    ...
    schema={
        "text": {
            ...
            "full_text_search": True,
            "regex": True,
            # Native embeddings. Check the model is available in your region:
            # https://turbopuffer.com/docs/embedding#models
            "embed": {"model": "nvidia/nemotron-3-embed-8b", "dims": 1024},
        },
        "category": {
            "type": "[]string",
...
func main() {
	...
	// Documents are updated by passing new data for an existing ID
	ns.Write(
		ctx,
		turbopuffer.NamespaceWriteParams{
			UpsertRows: []turbopuffer.RowParam{
...
public class QuickStart {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(queryResult);
    // NamespaceQueryResponse{rows=[{$dist=..., id=1, category=mammal}]}
    ...
    ns.write(
      NamespaceWriteParams.builder()
        .addUpsertRow(
          Row.builder().put("id", 1).put("text", "beluga whale").put("public", 1).build()
        )
        .addUpsertRow(Row.builder().put("id", 2).put("text", "manta ray").put("public", 1).build())
        .addUpsertRow(
          Row.builder().put("id", 3).put("text", "octopus cuttlefish").put("public", 1).build()
        )
        .distanceMetric(DistanceMetric.COSINE_DISTANCE)
        .build()
    );
...
await ns.Write(
    new NamespaceWriteParams
    {
        UpsertRows =
        [
            new Row().Set("id", 1).Set("text", "beluga whale").Set("public", 1),
            new Row().Set("id", 2).Set("text", "manta ray").Set("public", 1),
            new Row().Set("id", 3).Set("text", "octopus cuttlefish").Set("public", 1),
        ],
        DistanceMetric = DistanceMetric.CosineDistance,
    }
);
...
curl $TPUF_URL/v2/namespaces/$TPUF_NAMESPACE \
  ...
  -d '{
   "upsert_rows": [
     {"id": 1, "text": "updated walrus", "category": ["mammal"],
...
curl $TPUF_URL/v2/namespaces/$TPUF_BRANCH_NAMESPACE/query \
  ...
  -H "Authorization: Bearer $api_key" \
  -H 'Content-Type: application/json' \
  -d '{"rank_by": ["text", "ANN", ["Embed", "sea creature"]], "limit": 5}'
```
```python
...
# Query the branch independently
print(branch.query(
    rank_by=("text", "ANN", ("Embed", "sea creature")),
    limit=5,
))
```
```typescript
...
await branch.write({ branch_from_namespace: namespace });
...
```go
	...
	branchResult, err := branch.Query(ctx,
		turbopuffer.NamespaceQueryParams{
			RankBy: turbopuffer.NewRankByAnnExpr("text",
				turbopuffer.NewExprEmbed("sea creature")),
...
```java
    ...
    var branchResult = branch.query(
      NamespaceQueryParams.builder()
        .rankBy(RankBy.ann("text", Expr.embed("sea creature")))
        .limit(5)
        .build()
    );
...
var branchResult = await branch.Query(
    new NamespaceQueryParams { RankBy = RankBy.Ann("text", Expr.Embed("sea creature")), Limit = 5 }
);
