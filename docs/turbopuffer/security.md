...
This _also_ allows customer's customers to use their own KMS to encrypt their
...
turbopuffer supports organization-level IP allowlisting to restrict API access to approved IP
addresses or CIDR ranges. Requests reaching the organization through a public turbopuffer endpoint are rejected unless their source IP falls within an allowed range. IP allowlisting is currently available on the
[Scale and Enterprise](/pricing) plans.

[Contact us](/contact) to enable IP allowlisting for your organization.
...
<a id="subprocessors-for-customer-data"></a>

## Subprocessors

turbopuffer discloses every vendor that processes customer data, covering
infrastructure, platform, and optional feature subprocessors, on the dedicated
[subprocessors](/docs/security/subprocessors) page, along with a subscription for update
notifications.
