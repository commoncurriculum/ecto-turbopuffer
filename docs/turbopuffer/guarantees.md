...
* **Sharding preserves atomicity and consistency.** On a [sharded namespace](/docs/sharding), writes commit once to a shared WAL and apply to all shards simultaneously with all-or-nothing semantics. Strongly consistent queries read from a single snapshot across all shards; [eventually consistent queries](/docs/query#param-consistency) may serve results from different points in time per shard.
