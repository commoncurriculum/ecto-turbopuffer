...
Pinning is a per-namespace setting and is not carried over by
[`copy_from_namespace`](/docs/write#param-copy_from_namespace) or
[`branch_from_namespace`](/docs/branching). The destination starts unpinned;
pin it explicitly if desired.

Pinning composes with [namespace sharding](/docs/sharding): when a sharded
namespace is pinned, each shard is pinned individually to its own reserved query
nodes, and replicas apply per shard.
...
For [sharded namespaces](/docs/sharding), size is the namespace's total across
all shards, so the number of shards does not affect the cost of pinning.
...
Billing has a floor of **128 GB** and **10 minutes**. A pinned namespace
smaller than 128 GB is billed as 128 GB, and a namespace pinned for less than
10 minutes is billed for 10 minutes.
