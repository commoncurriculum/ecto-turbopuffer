...
Sharding partitions a single namespace across multiple internal shards, letting
it scale beyond the size limit of a single index. Clients keep talking to one
namespace: writes commit once to a shared [WAL](#wal), documents are assigned
to shards by hash of their id, and queries fan out across shards and are merged
transparently. See the [sharding guide](/docs/sharding) for details.
