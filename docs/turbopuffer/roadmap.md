...
- 📕 More full-text search features ([~rank by attribute~](#february-2026), [~rank by distance~](#february-2026), [~highlighting~](#july-2026), [~fuzzy search~](#may-2026), ...)
- ∑ More aggregate functions ([~count~](#may-2025), [~group by~](#august-2025), [~sum~](#november-2025), distinct, min, max...)
- ⏱️ [~Late interaction support~](#june-2026)
- 🗂️ [~Multiple vector columns~](#march-2026)
- 🌿 [~Namespace branching~](#may-2026)
- 🪓 Namespace sharding ([~fixed shard count~](#july-2026), manual resharding, automatic resharding)
- 🫛 [~Native embedding~](#july-2026)
- 🦆 Easy embedding model migrations
- 🪆 Nested attributes
...
- 🪓 [Namespace sharding](/docs/sharding) is generally available

### August 2026

- 🫛 Embed without an API call: [native embedding](/docs/embedding) is generally available
- 🧬 Added support for 4 new [embedding models](/docs/embedding/changelog#august-2026)
- 🗺️ Browse namespace documents and run queries [in the dashboard](https://x.com/turbopuffer/status/2098479494021697909)
- 🔟 Store arbitrary binary data in turbopuffer with the [`bytes` data type](/docs/write#param-type)
- ☁️ First public Azure region, [`azure-eastus2`](/docs/regions), now in private preview
- 🔒 Create [`read_only`](/docs/metadata#param-read_only) namespaces that reject new document and schema writes
- 🔢 Order results by [up to 8 attributes](/docs/query#ordering-by-attributes) to break ties on subsequent sort keys
- 🪣 Diversify [full-text search](/docs/query#diversification) results with [`limit.per`](/docs/query#param-limit)
- 🐫 Compute [multiple aggregations](/docs/query#aggregations) in a single query

### July 2026

- 🫛 Embed without an external API call using [native embedding](/docs/embedding) [opt-in, beta]
- 🪓 Scale a single namespace up to 256TB with [namespace sharding](/docs/sharding) [opt-in, beta]
- 🖍️ [Highlight](/docs/fts#highlighting) the specific passages in a document that match the query [beta]
- ⚖️ Fine tune the RRF reranker with a [`weight` per subquery](/docs/query#reciprocal-rank-fusion)
- 📶 Pass extra signals to your reranker with [`compute_attributes`](/docs/query#param-compute_attributes) on the results of a query
- ☁️ Pay for turbopuffer by drawing down your Google Cloud commit on the [Google Cloud Marketplace](https://console.cloud.google.com/marketplace/product/turbopuffer-marketplace/turbopuffer-saas)

### June 2026

- 💵 Launch plan minimum [reduced from $64 → $16/month](/docs/pricing-log)
- 🧮 [`i8` vector type](/docs/write#param-type) for 75% lower storage and query costs vs `f32`
- ⏱️ Store multiple vectors per doc and rerank with [late interaction](/docs/query#late-interaction) (private beta)
- 🌐 Limit public endpoint access to specific IP ranges with [IP allowlisting](/docs/security#ip-allowlisting)
- ⚛️ Combine multi-query results server-side with [`rerank_by`](/docs/query#param-rerank_by)
- 🪣 Diversify results on ANN and kNN queries using [`limit.per`](/docs/query#diversification)
- 🦥 [Case-insensitive](/docs/query#param-Fuzzy) fuzzy substring matching
