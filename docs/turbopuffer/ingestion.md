...
Note that `disable_backpressure` is not supported with `patch_rows`, `patch_columns`, `delete_by_filter`, `patch_by_filter`, or conditional writes, as these rely on strongly consistent reads.
